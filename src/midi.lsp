...
;;; Some additions to the midi-lib for use in comic:

(defmethod get-bpm ((msg tempo-message))
  (/ 60000000 (slot-value msg 'tempo)))
...
(defun get-time-in-microseconds (midi-time
  ...
  (if (> divisions 16383)
      ...
      (cc::cc-error 'Parse-midi-file
	  "Midi-time in FPS is not implemented.")
      (* midi-time
	 (/ microseconds-per-beat
	    divisions))))

(defun get-time-in-secs (midi-time
			 microseconds-per-beat
			 divisions)
...
(defun is-note-off (msg)
  ...
  velocity 0."
  (or (typep msg 'note-off-message)
      (and
       (typep msg
	      'note-on-message)
       (zerop (message-velocity msg)))))
